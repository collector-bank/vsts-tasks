...
import com.eviware.loadui.api.LoadUI

//Here to support Splitters created in loadUI 1.0, remove in the future:
try {
    renameProperty('outputs', 'numOutputs')
} catch (e) {
}
...
def compensateProbabilities(changedProperty, diff) {
    ...
    while (diff > 0) {
        indexToChange = latestChanged.find {
            it.value == latestChanged.findAll { terminalProbabilities[it.key].value != 0 }.collect { it.value }.min()
        }.key
    ...
    while (diff < 0) {
        indexToChange = latestChanged.find {
            it.value == latestChanged.findAll { terminalProbabilities[it.key].value != 100 }.collect { it.value }.min()
        }.key
...
compactLayout {
    box(widget: 'display', layout: 'wrap, fillx', constraints: 'growx') {
        ...
        node(label: 'Distribution', content: {
